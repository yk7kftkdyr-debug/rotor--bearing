...
if ~(numel(loads.nodes) == numel(loads.Fx) && numel(loads.nodes) == numel(loads.Fy)), error('static_load nodes, Fx and Fy must have equal lengths.'); end
for k = 1:numel(loads.nodes)
    node = loads.nodes(k); F(4*node-3) = F(4*node-3) + loads.Fx(k); F(4*node-2) = F(4*node-2) + loads.Fy(k);
end
end
...
for ib = 1:numel(params.bearing)
    ...
    iy = 4*b.rotor_node - 2; q(iy) = sign(sum(F_static(vertical_translation_dofs(modelInfo, cfg.vertical_direction))))*(max(c,0) + cfg.contact_seed);
end
end
...
dofs = [2:4:modelInfo.num_rotor_dof, modelInfo.num_rotor_dof+2:4:modelInfo.num_rotor_dof+modelInfo.num_case_dof].';
end
...
for ib = 1:numel(params.bearing)
    b = params.bearing(ib); irx = 4*b.rotor_node-3; iry = irx+1; icx = modelInfo.num_rotor_dof+4*b.case_node-3; icy = icx+1;
...
zero = params; zero.static_equilibrium.gravity = 0; zero.static_load.Fx(:) = 0; zero.static_load.Fy(:) = 0;
...
if isfield(params, 'case_ground_nodes')
    for node = params.case_ground_nodes
        iy = modelInfo.num_rotor_dof + 4*node - 2;
