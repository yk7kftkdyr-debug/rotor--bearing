function loadState = qiujieLOAD3(datafromvb)                % 牛顿法求解非线性方程组，考虑滚子歪斜
...
baseLoadState = qiujieLOAD(datafromvb);
loadj = baseLoadState.loadj;
loadi = baseLoadState.loadi;
www1 = baseLoadState.www1;
...
% 求解
www12=pp';result112=1e4; nn2=1; temp11=[];  afachange=1e-5; min2=1e50; wwmin2=www12;
for i=1:(3*loadj+4)
    ddd(i)=0;
end
while( result112>0.001 )
    f3 = ff3(datafromvb,loadi,www12);
    result112 = f3.result112;
    aaa=result112;
    if result112<min2
        wwmin2=www12;min2=result112
    end
    q2=www12;    
    for ij=1:loadj
        q2(2*loadj+ij)=www12(2*loadj+ij)*(1+afachange);
        f3Pert = ff3(datafromvb,loadi,q2);
        bbb=f3Pert.result112;
        ddd(2*loadj+ij)=(bbb-aaa)/(afachange*www12(2*loadj+ij));
        q2=www12;
    end
    q2(3*loadj+4)=www12(3*loadj+4)*(1+afachange);
    f3Pert = ff3(datafromvb,loadi,q2);
    bbb=f3Pert.result112;
    ddd(3*loadj+4)=(bbb-aaa)/(afachange*www12(3*loadj+4));
    q2=www12;
    dd=0;
    for j=1:(3*loadj+4)
        dd=dd+ddd(j)^2;
...
www12=wwmin2;  
finalContact = ff3(datafromvb,loadi,www12);

loadState = baseLoadState;
loadState.www12 = real(www12);
loadState.contact = finalContact;
loadState.result112 = finalContact.result112;
loadState.wwmin2 = wwmin2;
loadState.ddd = ddd;
