function [Fx, Fy, state] = ball_bearing_force(xr, yr, vxr, vyr, bearing, omega, t)
%BALL_BEARING_FORCE Ball bearing nonlinear point-contact force.
% Inputs:
%   xr, yr, vxr, vyr - relative displacement and velocity between rotor and case.
%   bearing          - ball bearing parameters.
...
Db = bearing.Db;
Dm = bearing.Dm;
alpha = get_field_default(bearing, 'contact_angle', 0);
mu0 = bearing.mu;
K_point = get_field_default(bearing, 'K_point', 4.0e9);
cn = get_field_default(bearing, 'contact_cn', get_field_default(bearing, 'contact_damping', 60));
...
omega_cage_ideal = 0.5*omega*(1 - Db/Dm*cos(alpha));
omega_ball_spin = Dm/(2*Db)*(1 - (Db*cos(alpha)/Dm)^2)*omega;
...
if get_field_default(waviness, 'enable', false)
    ...
    waviness_order = get_field_default(waviness, 'order', get_field_default(bearing, 'waviness_order', 2));
else
    P2_amp = 0;
    waviness_order = get_field_default(bearing, 'waviness_order', 2);
end

rho_b = get_field_default(bearing, 'ball_density', 7850);
m_ball = rho_b*pi*Db^3/6;
Fc = m_ball*(omega_cage_ideal^2)*(Dm/2);
...
for i = 1:n
    ci = cos(theta(i));
    si = sin(theta(i));
    h_uncorrected(i) = h_base*(1 + 0.05*cos(theta(i) - atan2(yr, xr)) + texture_gamma*cos(waviness_order*theta(i)));
    ...
    if delta(i) > 0
        vn = vxr*ci + vyr*si;
        Q_elastic = K_point*delta(i)^(3/2);
        Q_damping = 0;
        if contact.damping_enable
            Q_damping = cn*vn*Hs(i);
        end
        Qi = Q_elastic + Q_damping + 0.02*Fc*Hs(i);
        Qi = min(max(Qi, 0), Qmax);
        if ehl_enable
            ...
            Qi = min(max(Q_elastic + Q_damping + 0.02*Fc*Hs(i), 0), Qmax);
        end
        Qrad = Qi*cos(alpha);
        k_contact(i) = 1.5*K_point*max(delta(i), contact.smooth_delta)^(0.5)*Hs(i);

        tx = -si;
        ty = ci;
        vt = vxr*tx + vyr*ty - omega_cage_ideal*Db*0.02 + 0.01*omega_ball_spin*Db;
        lambda_ratio = h(i)/max(rough, 1e-9);
        traction_boost = 1 + 0.35*exp(-lambda_ratio) + 0.10*min(abs(vt)/U, 2);
        mu_i(i) = mu0*traction_boost;
        Ft = mu_i(i)*Qrad*sign_smooth(vt);

        Fx_i(i) = -Qrad*ci - Ft*tx;
        Fy_i(i) = -Qrad*si - Ft*ty;
        Q(i) = Qi;

        contact_area = max(pi*(0.20*Db)^2, 1e-12);
        p_mean = Qi/contact_area;
        PV(i) = p_mean*abs(vt);
        slip_i(i) = min(0.55, max(0, 0.015 + 0.25*Fc/(Qi + Fc + 1) + 0.035*abs(vt)/(U + 1e-12)));
    else
        if ehl_enable
...
h0 = get_field_default(bearing, 'oil_film0', 0.6e-6);
h = h0 + 1.8e-8*(mu_oil/3.18e-2)^0.67*(alpha_p/1.28e-8)^0.49*(U/10)^0.67;
h = min(max(h, 0), 4e-6);
end
