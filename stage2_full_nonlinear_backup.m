...
if isfield(params, 'case_definition') && isfield(params.case_definition, 'name') && ...
        strcmp(params.case_definition.name, 'stage1_static_equilibrium_ball_roller')
...
% Legacy linear support can be retained for comparison only. Stage 1 uses
% nonlinear contact directly so K_s stays free of duplicated bearing stiffness.
if get_field_default(params, 'use_legacy_linear_bearing_coupling', false)
...
if isfield(params, 'static_equilibrium_result') && isfield(params.static_equilibrium_result, 'q0')
    yn(:,1) = params.static_equilibrium_result.q0;
end
...
F_ext = zeros(N3, 1);
if isfield(params, 'static_equilibrium_result') && isfield(params.static_equilibrium_result, 'F_static')
    F_ext = params.static_equilibrium_result.F_static;
    return;
end
