...
% Four equivalent impeller unbalances.
...
params.unbalance.phase_mode = 'distributed';
...
% Quasi-dynamic bearing preloads. Final baseline values are assigned by
% apply_official_case_config() at the end of this file so there is only one
% official source for the paper baseline.
params.bearing(1).preload_y = 0;
params.bearing(1).preload_z = 2000;
params.bearing(2).preload_y = 0;
params.bearing(2).preload_z = 30000;
...
% Official baseline used by main.m:
% bearing1 front ball = 20 kN, bearing2 rear roller = 80 kN.
% The loads are quasi-dynamic bearing preloads only; rotor-node static_load
% is fixed to zero for the baseline.
