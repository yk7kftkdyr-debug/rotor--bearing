...
test_shared_frozen_input_projection_contract(repository_root);
...
function test_shared_frozen_input_projection_contract(repository_root)
canonical = fullfile(repository_root,'results', ...
    'thermal_equivalent_damping_frequency_domain', ...
    'thermal_4T_frequency_domain_results.mat');
artifact = load(canonical); baseline = artifact.results;
projection = build_stage_f_frozen_input_projection(baseline);
assert(isequal(sort(fieldnames(projection)), ...
    {'C_formal';'schema_version';'temperature_cases'}));
assert(strcmp(projection.schema_version, ...
    'stage-f-frozen-input-projection-v1'));
temperature_fields = {'T20','T50','T80','T100'};
temperature_required = { ...
    'Fhat_source_sha256','J_front_sha256','J_rear_sha256', ...
    'K_t_sha256','clearance_sha256','contact_state_front_sha256', ...
    'contact_state_rear_sha256','k_static_front_sha256', ...
    'k_static_rear_sha256','loaded_mask_front_sha256', ...
    'loaded_mask_rear_sha256','qhat_source_sha256', ...
    'static_state_summary_sha256','thermal_state_sha256'};
for k = 1:numel(temperature_fields)
    tf = temperature_fields{k}; item = projection.temperature_cases.(tf);
    assert(isequal(sort(fieldnames(item)),sort(temperature_required(:))));
    for scalar_hash = setdiff(temperature_required, ...
            {'Fhat_source_sha256','qhat_source_sha256'})
        assert(valid_sha256(item.(scalar_hash{1})));
    end
    assert(isequal(sort(fieldnames(item.qhat_source_sha256)), ...
        {'HIGH';'LOW';'NOMINAL'}));
    assert(isequal(sort(fieldnames(item.Fhat_source_sha256)), ...
        {'HIGH';'LOW';'NOMINAL'}));
    for scenario = {'LOW','NOMINAL','HIGH'}
        assert(valid_sha256(item.qhat_source_sha256.(scenario{1})) && ...
            valid_sha256(item.Fhat_source_sha256.(scenario{1})));
    end
end
assert(isequal(sort(fieldnames(projection.C_formal)), ...
    {'HIGH';'LOW';'NOMINAL'}));
assert(all(structfun(@valid_sha256,projection.C_formal)));
assert(~recursive_field_present(projection,'progress') && ...
    ~recursive_field_present(projection,'validation') && ...
    ~recursive_field_present(projection,'decision') && ...
    ~recursive_field_present(projection,'timestamp') && ...
    ~recursive_field_present(projection,'stage_f_complete'));

flow_changed = baseline;
flow_changed.progress.stage_f_complete = ~baseline.progress.stage_f_complete;
flow_changed.validation.stage_f = struct('passed',false);
flow_changed.decision.status = 'FLOW_ONLY_MUTATION';
assert(isequaln(build_stage_f_frozen_input_projection(flow_changed),projection), ...
    'StageF:FrozenProjectionContainsFlowState', ...
    'Progress, validation and decision must not affect the frozen input projection.');

scientific_changed = baseline;
scientific_changed.frequency_response.T20.NOMINAL.qhat_full(1) = ...
    scientific_changed.frequency_response.T20.NOMINAL.qhat_full(1)+eps;
assert(~isequaln(build_stage_f_frozen_input_projection(scientific_changed),projection), ...
    'StageF:FrozenProjectionMissedQhat', ...
    'A Stage E qhat source mutation must change the frozen input projection.');
end

function pass = valid_sha256(value)
pass = ischar(value) && ...
...
candidate.stage_f_execution_audit.canonical_input_reference = ...
    stage_f_input_reference_fixture( ...
    build_stage_f_frozen_input_projection(baseline));
validation = validate_stage_f_results(candidate,baseline);
...
subject = 'fix: unify stage f frozen input projection';
...
assert(contains(transaction,'build_stage_f_frozen_input_projection') && ...
    ~contains(transaction,'stage_e_projection'), ...
    'StageF:SharedFrozenProjection', ...
    'Independent validation must use the shared frozen projection, not construct its own projection.');
assert(contains(validator,'build_stage_f_frozen_input_projection') && ...
    contains(runner,'build_stage_f_frozen_input_projection'), ...
    'StageF:SharedFrozenProjection', ...
    'Runner and validator must call the same frozen input projection function.');
end
