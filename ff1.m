function loadn=ff1(datafromvb )   % 求解 承载滚动体数目！！
% 数据输入部分！！
...
denom = 2*deltamax + deltar0;
if denom <= eps || ~isreal(denom)
    loadcoeff1 = 1;
    loadn = floor((n-1)/2);
else
    loadcoeff1 = deltamax/denom;
    loadAngleRatio = real(deltar0/denom);
    loadAngleRatio = max(-1,min(1,loadAngleRatio));
    loadangle1 = acos(loadAngleRatio);
    loadn = floor(n*loadangle1/(2*pi));
    loadn = max(0,min(floor((n-1)/2),loadn));
end
temp1=0;
for i=1:loadn                    % 这里不包括0号滚子.
    sita(i)=2*pi*i/n;  %求各个滚子的方位角  
    loadTerm = 1-(1-cos(sita(i)))/(2*loadcoeff1);
    if loadTerm > 0
        temp1=temp1+loadTerm^(10/9)*cos(sita(i));
    end
end
